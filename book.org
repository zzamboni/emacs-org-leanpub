...
| =#+begin/end_note=        | ={blurb, class: "information", icon: "sticky-note"}= |
| =#+begin/end_question=    | ={blurb, class: "question"}=                         |
| =#+begin/end_tip=         | ={blurb, class: "tip"}=                              |
| =#+begin/end_warning=     | ={blurb, class: "warning"}=                          |

#+begin_note
The "Note" environment is not natively supported by Leanpub, but it is supported by Org, so =ox-leanpub= renders it as a =blurb= with a custom icon, like the one you see here.
#+end_note
...
You can see a sample of all the default blocks in [[#appendix-block-types][Appendix A]].
