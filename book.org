...
The trigger definition under =on:= is the first important part. It says that the workflow should run for pushes to =master= and also for any pushed tag whose name starts with =publish-=. This mirrors the convention we discussed above: ordinary work produces previews, and release tags produce publications.

The =Preview Book= step is the simple case. If the ref that triggered the workflow is =refs/heads/master=, the action is called with =action: "preview"=. The only required parameters are the Leanpub API key, stored in the repository secret =LEANPUB_API_KEY=, and the book slug.

The publish path is slightly more interesting. If the pushed ref is a tag starting with =publish=, the workflow first checks out the repository and inspects the tag with Git itself. When the tag is annotated, it extracts the annotation contents and stores them as a step output named =release_notes=. Later, the final publish step passes those contents to Leanpub using the =release-notes= parameter.
...
If the tag is /not/ annotated, the workflow still publishes the book, but without release notes. In both publish cases it sets =email-readers: "true"=, which instructs Leanpub to notify readers of the new version.
