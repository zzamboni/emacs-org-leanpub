...
You can specify a [[https://leanpub.com/markua/read#leanpub-auto-using-extension-attributes-on-blurbs-to-add-icon-support][custom icon]] for a block using the =:icon= attribute in an =#+ATTR_LEANPUB= line:

#+begin_src org
,#+ATTR_LEANPUB: :icon lightbulb
,#+begin_tip
Tip with a lightbulb icon instead of the default.
,#+end_tip
#+end_src
...
At least one of =:class= or =:icon= needs to be specified.

For example, we could change the default icon of =tip= blocks to be a checkmark:

#+begin_src org
,#+MARKUA_BLOCK: tip :class tip :icon check

,#+begin_tip
Tip with a checkmark!
,#+end_tip
#+end_src

#+attr_leanpub: :icon check
#+begin_tip
Tip with a checkmark!
#+end_tip

We could also define a completely new block type:
