...
(let* ((blocks (cl-sort (cl-copy-seq org-leanpub-markua--block-definitions)
...
|------------+------------------|\n"
   (mapconcat
    (lambda (block-def)
      (cl-destructuring-bind (org-block markua-block markua-attrs) block-def
        (format "| =#+begin/end_%s= | =%s= |"
                org-block
                (org-leanpub-markua--attribute-line-string
                 markua-block markua-attrs))))
    blocks
    "\n")))
#+end_src
...
#+begin_tip
You can see a sample of all these default blocks in [[#appendix-block-types][Appendix A]].
#+end_tip
...
Org has extensive facilities for [[https://orgmode.org/manual/Working-with-Source-Code.html#Working-with-Source-Code][working with source code]]. By far, any code blocks you include in your Org document will be properly recognized and typeset by Leanpub.
*** Book sample
...
,#+end_%s" b (upcase-initials b) b b b))
 (sort (mapcar #'car org-leanpub-markua--block-definitions)
       #'string-lessp)
 "\n")
#+end_src
...
#+begin_code
The block samples in this appendix are generated programmatically by the code shown in [[*Code block execution and output processing][Code block execution and output processing]]!
#+end_code
